...
public class RecyclerViewChallengedFriendsAdapter extends RecyclerView.Adapter<RecyclerViewChallengedFriendsAdapter.FriendsRecyclerViewHolder> {

    private Context mContext;
    private ArrayList<ChallengedFriendsModel> friendsList;
    private Utils utils;
    ...
    public RecyclerViewChallengedFriendsAdapter(Context context, ArrayList<ChallengedFriendsModel> friendsList, boolean isProfile) {
    ...
    public FriendsRecyclerViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        ...
        ViewGroup mainGroup = (ViewGroup) mInflater.inflate(R.layout.challenged_item_row_friends, viewGroup, false);
        return new FriendsRecyclerViewHolder(mainGroup);
    }

    @Override
    public void onBindViewHolder(FriendsRecyclerViewHolder holder, int position) {
        final ChallengedFriendsModel model = friendsList.get(position);
...
//        String name = model.getName().substring(0, 1).toUpperCase()+model.getName().substring(1);
        ...
        if (!isProfile) {
            ...
            holder.challengedDateFrom.setText(model.getChallengedStartDate());
            holder.challengedDateTo.setText(model.getChallengedEndDate());
//            if(model.isWeight()){
//               holder.challengedScoreFriendIcon.setCompoundDrawablesWithIntrinsicBounds(ContextCompat.getDrawable(mContext, R.drawable.ic_weight), null, null, null);
//               holder.challengedScoreUserIcon.setCompoundDrawablesWithIntrinsicBounds(ContextCompat.getDrawable(mContext, R.drawable.ic_weight), null, null, null);
//
//            }else{
//                holder.challengedScoreFriendIcon.setCompoundDrawablesWithIntrinsicBounds(ContextCompat.getDrawable(mContext, R.drawable.ic_heart), null, null, null);
//                holder.challengedScoreUserIcon.setCompoundDrawablesWithIntrinsicBounds(ContextCompat.getDrawable(mContext, R.drawable.ic_heart), null, null, null);
//            }

            StartDate = model.getChallengedStartDate();
            EndDate = model.getChallengedEndDate();
            IsWeight = model.isWeight();


            ParseObject po = ParseObject.createWithoutData("_User", model.getObjectId());
            friendScore = populateScore(po, StartDate, EndDate, holder.challengedScoreFriend);
            userScore = populateScore(ParseUser.getCurrentUser(), StartDate, EndDate, holder.challengedScoreUser);
            ...
            if(friendScore == userScore){
                holder.challengedYouLoseWinDraw.setText("You Draw");

            }else if(friendScore > userScore){
                holder.challengedYouLoseWinDraw.setText("You Lose");

            }else if(friendScore < userScore){
                holder.challengedYouLoseWinDraw.setText("YOU WIN!!!");
            }
...
//                    holder.challengeButtonInside.setText(R.string.view_challenge);
//                } else {
//                    holder.challengeButtonInside.setText("CHALLENGE!");
//                }
//            } catch (ParseException e) {
//                e.printStackTrace();
//            }

//            holder.challengeButtonInside.setOnClickListener(new View.OnClickListener() {
//                @Override
...
//            });
    ...
    public void updateList(ArrayList<ChallengedFriendsModel> temp) {
...
//    add by chang end
    ...
    private int populateScore(final ParseObject currentUser, String StartDate, String EndDate, final TextView score){

        List<String> dates = utils.getDates(StartDate, EndDate);
...
//            public void done(List<ParseObject> newObjects, ParseException e) {
            try{
                ...
                List<ParseObject> newObjects = query.find();
...
//                if (e == null) {
                    if (IsWeight) {
                        ...
                        score.setText(String.valueOf(tAmount[0]));
                        score.setCompoundDrawablesWithIntrinsicBounds(ContextCompat.getDrawable(mContext, R.drawable.ic_weight), null,null,null);
                    } else {
                        for (ParseObject j : newObjects) {
                            tAmount[0] = tAmount[0] + j.getInt(TRACKED_HEARTS);
                        }
                        score.setText(String.valueOf(tAmount[0]));
                        score.setCompoundDrawablesWithIntrinsicBounds(ContextCompat.getDrawable(mContext, R.drawable.ic_heart), null, null, null);
                    }
    }catch(ParseException e) {
                e.printStackTrace();
            }
...
//        });
    ...
    public interface onFriendsListItemClickListener {
        void onItemClickListener(View view, int position, ChallengedFriendsModel model);
    }
    ...
    public void update(ArrayList<ChallengedFriendsModel> updateList) {
        friendsList.clear();
...
//    }
    ...
    class FriendsRecyclerViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public TextView name, username, challengedDateFrom, challengedDateTo, challengedScoreUserIcon, challengedScoreFriendIcon, challengedScoreFriend, challengedScoreUser, challengedYouLoseWinDraw;

        FriendsRecyclerViewHolder(View itemView) {
            super(itemView);
            this.profilePicture = (CircleImageView) itemView.findViewById(R.id.challenged_friends_profile_picture);
            this.name = (TextView) itemView.findViewById(R.id.challenged_friends_name);
            this.username = (TextView) itemView.findViewById(R.id.challenged_friends_username);
//            this.challengeButtonInside = (TextView) itemView.findViewById(R.id.challenged_friends_button_inside);
            this.challengedDateFrom = (TextView) itemView.findViewById(R.id.challenged_date_from_start);
            this.challengedDateTo = (TextView) itemView.findViewById(R.id.challenged_date_to_end);
            this.challengedScoreFriendIcon = (TextView) itemView.findViewById(R.id.challenged_score_friend_icon);
            this.challengedScoreUserIcon = (TextView) itemView.findViewById(R.id.challenged_score_user_icon);
            this.challengedScoreFriend = (TextView) itemView.findViewById(R.id.challenged_score_friend);
            this.challengedScoreUser = (TextView) itemView.findViewById(R.id.challenged_score_user);
            this.challengedYouLoseWinDraw = (TextView) itemView.findViewById(R.id.challenged_you_lose_win_draw);
        ...
        confirmButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                newRequest.saveInBackground(new SaveCallback() {
                    ...
                    public void done(ParseException e) {
                        utils.makeText(mContext.getResources().getString(R.string.successchallengerequest) + " " + name + ".", LENGTH_LONG);
//                        SashidoHelper.earnAchievement(CHALLENGE_BEGIN_COL, mContext.getString(R.string.challenge_begin), mContext, getView());      ?????????????
                        dialog.dismiss();
                    }
                });
            }
