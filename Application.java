...
public class Application extends android.app.Application {
    ...
    private void initSDKs() {
        if (Locale.getDefault().getCountry().equals("AU")) {
            SharedPreferencesManager.setString(mContext, "locale", "au");
            Parse.initialize(new Parse.Configuration.Builder(mContext)
                    .applicationId(Constants.SASHIDO_APP_ID_AUS)
                    .clientKey(Constants.SASHIDO_CLIENT_ID_AUS)
                    .server(Constants.SASHIDO_SERVER_ID_AUS)
                    .build()
            );
            ParsePush.subscribeInBackground("Proludic AUS", new SaveCallback() {
                @Override
