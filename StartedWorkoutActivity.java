...
public class StartedWorkoutActivity extends AppCompatActivity {
    ...
    private boolean isOneTouchWorkOut;
    private LinearLayout progressBar;
    private OneTouchProgressAnimation progressBarAnimation;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        isOneTouchWorkOut = bundle.getBoolean("isOneTouchWorkOut");
        if(isOneTouchWorkOut) {
            System.out.println("One Touch Work Out");
            progressBar = (LinearLayout) findViewById(R.id.one_touch_workout_progress_bar);
            progressBar.setVisibility(View.VISIBLE);
        } else {
            System.out.println("Main Work Out");
        }
        ...
        if(isOneTouchWorkOut) {
            startWorkoutButton.setOnClickListener(customListenerOneTouch);
        } else {
    ...
    private View.OnClickListener customListenerOneTouch = new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            switch (v.getId()) {
                case R.id.tap_to_start:
                    if (startWorkoutButton.getText().toString().equals(context.getResources().getString(R.string.taptostart))) {
                        workoutTimerTV.setTextColor(Color.parseColor("#00cc00"));
                        startWorkoutButton.setText("NEXT EXERCISE");
                        recyclerView.findViewHolderForAdapterPosition(0).itemView.findViewById(R.id.el_container).setBackgroundColor(ContextCompat.getColor(context, R.color.colorPrimaryHalf));
                        displayExerciseTime(0);
                        startMainTimer(true);
                        startMainTimer(false);
                        animateOneTouchProgressBar(0);
                    } else if (startWorkoutButton.getText().toString().equals("NEXT EXERCISE")) {
                        /*finishOneExercise();
                        if(count < totalExercises) {
                            nextExercise(5);
                        }*/
    ...
    public void nextExercise(int waitingTime) {
        progressBar.clearAnimation();
        if(waitingTime > 0) {
            startWorkoutButton.setText("BE READY FOR NEXT EXERCISE");
        } else {
        ...
        workoutTimerTV.setTextColor(Color.parseColor("#00cc00"));
        animateOneTouchProgressBar(waitingTime * 1000);
    }
...
private boolean isStopped = false;
    private void animateOneTouchProgressBar(final long waitingTime) {
        final long duration;
        final int startWidth;
        final int targetWidth;
        if(waitingTime == 0) {
            if(count > 1) {
                duration = exerciseList.get(count - 1).getTotalMillis();
            } else {
                duration = exerciseList.get(0).getTotalMillis();
            }
        ...
        progressBarAnimation = new OneTouchProgressAnimation(progressBar, targetWidth, startWidth
        );
        progressBarAnimation.setAnimationListener(new Animation.AnimationListener(){
            ...
            public void onAnimationEnd(Animation arg0) {
                if(waitingTime == 5 * 1000) {
                    nextExercise(0);
                } else if(waitingTime == 30 * 1000) {
                    nextExercise(0);
                } else {
                    if(!isStopped) {
                        finishOneExercise();
    ...
    public class OneTouchProgressAnimation extends Animation {
        final int targetWidth;
        View view;
        int startWidth;
        boolean isIncreasingAnimation;

        public OneTouchProgressAnimation(View view, int targetWidth, int startWidth) {
            this.view = view;
            this.targetWidth = targetWidth;
            this.startWidth = startWidth;
            if(targetWidth < startWidth) {
                isIncreasingAnimation = false;
            } else {
                isIncreasingAnimation = true;
            }
        }
